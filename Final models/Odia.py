...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/ori_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Odia")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/ori.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/ori.csv")
...
def main():
    print("Starting Odia Specialist Processing (L3Cube-Odia)...")
    df_train, df_val, df_test = load_data()

    model_name = "l3cube-pune/odia-bert"
    ...
    try:
        ...
        print(f"Error loading L3Cube: {e}. Falling back to MuRIL.")
    ...
    pred_path = os.path.join(OUTPUT_DIR, "pred_ori.csv")
    pd.DataFrame({'id': df_test['id'], 'polarization': labels_final}).to_csv(pred_path, index=False)

    prob_path = os.path.join(OUTPUT_DIR, "probs_ori.csv")
    pd.DataFrame({'id': df_test['id'], 'prob_1': probs}).to_csv(prob_path, index=False)

    print(f"Odia processing complete. Results saved to {pred_path}")
