...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/nep_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Nepali")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/nep.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/nep.csv")
...
def main():
    print("Starting Nepali Specialist Processing (mDeBERTa V3 Base)...")
    ...
    print("Saving final Nepali model...")
    ...
    pred_path = os.path.join(OUTPUT_DIR, "pred_nep.csv")
    pd.DataFrame({"id": df_test["id"], "polarization": labels_final}).to_csv(pred_path, index=False)

    prob_path = os.path.join(OUTPUT_DIR, "probs_nep.csv")
    pd.DataFrame({"id": df_test["id"], "prob_1": probs}).to_csv(prob_path, index=False)

    print(f"Nepali processing complete. Results saved to {pred_path}")
