...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/hau_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Hausa")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/hau.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/hau.csv")
...
def main():
    print("Starting Hausa Specialist Processing...")
    ...
    print("Saving final Hausa model...")
    trainer.save_model(MODEL_SAVE_DIR)
    tokenizer.save_pretrained(MODEL_SAVE_DIR)

    print("Generating predictions with threshold 0.35...")
    preds = trainer.predict(test_ds)

    probs = torch.nn.functional.softmax(torch.tensor(preds.predictions), dim=-1)[:, 1].numpy()
    labels_final = (probs >= THRESHOLD).astype(int)

    pred_path = os.path.join(OUTPUT_DIR, "pred_hau.csv")
    pd.DataFrame({"id": df_test["id"], "polarization": labels_final}).to_csv(pred_path, index=False)

    probs_path = os.path.join(OUTPUT_DIR, "probs_hau.csv")
