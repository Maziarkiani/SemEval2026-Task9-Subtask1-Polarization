...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/mya_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Burmese")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/mya.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/mya.csv")
...
def main():
    print("Starting Burmese Specialist Processing (mDeBERTa V3 Base)...")
    ...
    # Note: Gradient checkpointing is NOT enabled based on S8 stability notes
    ...
    # S8 Stability Settings: Batch 4, Accumulation 4 (Effective Batch 16)
    args = TrainingArguments(
        output_dir="./temp_mya_deberta",
        eval_strategy="epoch",
        save_strategy="epoch",
    ...
    print("Saving final Burmese model...")
    ...
    pred_path = os.path.join(OUTPUT_DIR, "pred_mya.csv")
    pd.DataFrame({'id': df_test['id'], 'polarization': labels_final}).to_csv(pred_path, index=False)
    
    prob_path = os.path.join(OUTPUT_DIR, "probs_mya.csv")
    pd.DataFrame({'id': df_test['id'], 'prob_1': probs}).to_csv(prob_path, index=False)
    
    print(f"Burmese processing complete. Results saved to {pred_path}")
