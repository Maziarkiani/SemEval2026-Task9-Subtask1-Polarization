...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/fas_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Persian")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/fas.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/fas.csv")
...
def train_and_predict(model_name, alias, df_train, df_val, df_test, use_fast_tok=True):
    ...
    # Master alignment: mDeBERTa uses use_fast=False
    if alias == "deberta":
...
def main():
    print("Starting Persian Ensemble Processing...")
    df_train, df_val, df_test = load_data()

    probs_parsbert = train_and_predict(
    ...
    print("Ensembling predictions and applying 0.60 threshold...")
    final_probs = (probs_parsbert * 0.60) + (probs_deberta * 0.40)

    labels_final = (final_probs >= 0.60).astype(int)

    out_df = pd.DataFrame({"id": df_test["id"], "polarization": labels_final})
    pred_path = os.path.join(OUTPUT_DIR, "pred_fas.csv")
