...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/hin_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Hindi")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/hin.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/hin.csv")
...
def train_and_predict(model_name, alias, df_train, df_val, df_test):
    ...
    # Keep save_strategy="epoch" (master uses "no")
    if alias == "deberta":
...
def main():
    print("Starting Hindi Ensemble Processing...")
    df_train, df_val, df_test = load_data()

    probs_deberta = train_and_predict(
        "microsoft/mdeberta-v3-base",
        "deberta",
        df_train, df_val, df_test
    )
    probs_l3cube = train_and_predict(
        "l3cube-pune/hate-multi-roberta-hasoc-hindi",
        "l3cube",
        df_train, df_val, df_test
    )

    print("Ensembling predictions and applying threshold 0.60...")
    final_probs = (probs_deberta + probs_l3cube) / 2.0
    labels_final = (final_probs >= 0.60).astype(int)

    pred_path = os.path.join(OUTPUT_DIR, "pred_hin.csv")
    pd.DataFrame({"id": df_test["id"], "polarization": labels_final}).to_csv(pred_path, index=False)

    probs_path = os.path.join(OUTPUT_DIR, "probs_hin.csv")
    pd.DataFrame({"id": df_test["id"], "prob1": final_probs}).to_csv(probs_path, index=False)

    print(f"Predictions successfully saved to {pred_path}")
    print(f"Probabilities successfully saved to {probs_path}")
