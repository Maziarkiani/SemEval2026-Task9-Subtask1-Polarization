...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/khm_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Khmer")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/khm.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/khm.csv")
...
def main():
    print("Starting Khmer Specialist Processing (Metythorn Native Model)...")
    ...
    print("Saving final Khmer model...")
    ...
    pred_path = os.path.join(OUTPUT_DIR, "predkhm.csv")
    pd.DataFrame({"id": df_test["id"], "polarization": labels_final}).to_csv(pred_path, index=False)

    prob_path = os.path.join(OUTPUT_DIR, "probskhm.csv")
    pd.DataFrame({"id": df_test["id"], "prob1": probs}).to_csv(prob_path, index=False)

    print(f"Khmer processing complete. Results saved to {pred_path}")
