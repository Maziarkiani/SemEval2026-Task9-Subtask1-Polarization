...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/rus_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Russian")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/rus.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/rus.csv")
...
def train_and_predict(model_name, alias, df_train, df_val, df_test, use_fast_tok=True, batch_size=16):
...
def main():
    print("Starting Russian Ensemble Processing...")
    df_train, df_val, df_test = load_data()

    probs_rubert = train_and_predict(
    ...
    probs_mdeberta = train_and_predict(
