...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/swa_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Swahili")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/swa.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/swa.csv")
...
def main():
    print("Starting Swahili Specialist Processing (mDeBERTa-v3)...")
    ...
    print("Saving final Swahili model...")
    ...
    pred_path = os.path.join(OUTPUT_DIR, "pred_swa.csv")
    pd.DataFrame({'id': df_test['id'], 'polarization': labels_final}).to_csv(pred_path, index=False)
    
    prob_path = os.path.join(OUTPUT_DIR, "probs_swa.csv")
