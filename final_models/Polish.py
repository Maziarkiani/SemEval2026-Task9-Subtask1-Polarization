...
OUTPUT_DIR = os.path.join(BASE_DIR, "submissions/pol_final")
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "saved_models/Polish")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)

TRAIN_FILE = os.path.join(BASE_DIR, "merged/pol.csv")
TEST_FILE = os.path.join(BASE_DIR, "test/pol.csv")
...
def train_and_predict(model_name, alias, df_train, df_val, df_test, num_epochs, use_fast_tok=True):
...
def main():
    print("Starting Polish Ensemble Processing...")
    df_train, df_val, df_test = load_data()

    # Master Polish: XLM-R = 4 epochs, HerBERT = 5 epochs
    probs_xlmr = train_and_predict("xlm-roberta-base", "xlmr", df_train, df_val, df_test,
                                  num_epochs=4, use_fast_tok=True)
    probs_herbert = train_and_predict("allegro/herbert-base-cased", "herbert", df_train, df_val, df_test,
                                     num_epochs=5, use_fast_tok=True)

    print("Ensembling predictions and applying 0.45 threshold...")
    final_probs = (probs_xlmr + probs_herbert) / 2.0
    labels_final = (final_probs >= 0.45).astype(int)

    out_df = pd.DataFrame({'id': df_test['id'], 'polarization': labels_final})
    pred_path = os.path.join(OUTPUT_DIR, "pred_pol.csv")
